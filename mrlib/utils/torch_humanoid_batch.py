...
class Humanoid_Batch:
    ...
    def __init__(self, cfg, device=torch.device("cpu")):
        ...
        if not os.path.exists(mjcf_path):
            raise FileNotFoundError(
                f"Could not find mjcf file: {mjcf_path} (resolved from {self.mjcf_file})"
            )

        tree = parse(
            BytesIO(open(mjcf_path, "rb").read()),
            parser=parser,
        )
        self.dof_axis = []
        joints = sorted(
    ...
    def load_mesh(self):
        ...
        for asset in all_mesh:
            ...
            if not body is None and "mesh" in body.find("geom").attrib:
                ...
                scale = np.fromstring(
                    asset.attrib.get("scale", "1 1 1"), dtype=float, sep=" "
                )
                ...
                if not os.path.exists(file_path):
                    logging.warning(
                        f"Mesh file not found for {body_name} at {file_path}"
                    )
                    continue
                ...
                pos = np.fromstring(
                    geom.attrib.get("pos", "0 0 0"), dtype=float, sep=" "
                )
                quat = np.fromstring(
                    geom.attrib.get("quat", "1 0 0 0"), dtype=float, sep=" "
                )

                mat = sRot.from_quat(quat[[1, 2, 3, 0]]).as_matrix()
                self.body_mesh[body_name] = {
                    "vertices": (mesh_obj.vectors @ mat.T * scale + pos),
                    "faces": np.arange(len(mesh_obj.vectors.reshape(-1, 3))).reshape(
                        -1, 3
                    ),
                }
    ...
    def mesh_fk(self, pose=None, trans=None):
            ...
            for t in range(T):
                for idx, body_name in enumerate(self.body_names_augment):
                    if body_name in self.body_mesh:
                        ...
                        trans_vertices = (
                            vertices @ global_rot[b, t, idx].T.numpy()
                            + global_trans[b, t, idx].numpy()
                        )
        ...
        if not all_vertices:
            raise ValueError(
                "Could not generate any mesh vertices. Check if STL files are correctly referenced and loaded."
            )
        ...
        if not isinstance(mesh, o3d.geometry.TriangleMesh):
            ...
            logging.error(
                "Final mesh object is not a TriangleMesh. Returning FK translations."
            )
            return fk_res.global_translation_extend
